...
public class PlanDefinitionDaoImpl extends AbstractDao implements PlanDefinitionDao {
  ...
  public List<DafPlanDefinition> search(SearchParameterMap theMap) {
    ...
    buildVersionCriteria(theMap,criteria);
    return criteria.list();
  }
  ...
  private void buildVersionCriteria(SearchParameterMap theMap, Criteria criteria) {
	    List<List<? extends IQueryParameterType>> list = theMap.get("version");
	    if (list != null)
	      for (List<? extends IQueryParameterType> values : list) {
	        for (IQueryParameterType params : values) {
	          StringParam version = (StringParam)params;
	          if (version.getValue() != null)
	            criteria.add(Restrictions.sqlRestriction("{alias}.data->>'version' = '" + version.getValue() + "'")); 
	        } 
	      }  
	  }
}
