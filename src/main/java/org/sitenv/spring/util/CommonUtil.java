...
public class CommonUtil {
	...
	public OperationOutcome validateResource(Resource resource,String validatorEndpoint,FhirContext r4Context) {
		...
		try {
			System.out.println("In CommonUtil Validate Resource Method");
			RestTemplate restTemplate = new RestTemplate();
			HttpHeaders headers = new HttpHeaders();
			//headers.add("Accept", MediaType.APPLICATION_JSON_VALUE);
			System.out.println("Before r4Context");
			System.out.println(r4Context);
			String request = r4Context.newJsonParser().encodeResourceToString(resource);
			System.out.println(validatorEndpoint);
			...
			System.out.println("Response from Validator : " + response.getStatusCode());	
		}catch(Exception e) {
